...
-- ├──────────────┼─────────────────────┼────────────────────────────────────┤
-- │ 批次生成时机    │ 原料到货入库时        │ 生产工单完成入库时                   │
-- │ 触发操作       │ 仓库收货员填入库单     │ 工单报工完成 → 成品入库              │
...
-- └──────────────┴─────────────────────┴────────────────────────────────────┘
--
-- === 批次生成三步流程（参考 ktg-mes WmBatchServiceImpl.getOrGenerateBatchCode）===
-- ① 验证：读本表配置 → 检查 Y 属性是否都传了值 → N 属性强制置 null
-- ② 查重：按"Y 属性组合"查 wm_batch 表 → 找到则返回已有批次号（不重复建）
-- ③ 生成：没找到 → 调用编码器生成新 batch_code → INSERT wm_batch → 生成条码
--
-- ⚠️ 关键：批次号是流水号，批次身份是属性组合。同物料+同属性组合=同一批次。
--        因此入库员不需要手动编批次号，只需填好追踪属性，系统自动匹配或生成。
-- ----------------------------
