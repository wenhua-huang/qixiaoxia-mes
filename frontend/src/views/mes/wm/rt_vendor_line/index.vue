<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5"><el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['mes:wm:rt_vendor_line:add']">新增</el-button></el-col>
      <el-col :span="1.5"><el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['mes:wm:rt_vendor_line:edit']">修改</el-button></el-col>
      <el-col :span="1.5"><el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['mes:wm:rt_vendor_line:remove']">删除</el-button></el-col>
      <el-col :span="1.5"><el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['mes:wm:rt_vendor_line:export']">导出</el-button></el-col>
    ...
    <el-table v-loading="loading" :data="rtvendorlineList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="160">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['mes:wm:rt_vendor_line:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['mes:wm:rt_vendor_line:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="600px" append-to-body>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="退货数量" prop="quantityRt">
          <el-input v-model="form.quantityRt" placeholder="请输入" clearable />
        </el-form-item>
...
const rtvendorlineList = ref<WmRtVendorLine[]>([])
...
const data = reactive({
  ...
  queryParams: { pageNum: 1, pageSize: 10 } as WmRtVendorLineQueryParams,
  rules: {}
})
...
function reset() { form.value = {} as WmRtVendorLine; proxy.resetForm('formRef') }
...
function handleAdd() { reset(); open.value = true; title.value = '新增采购退货单行表' }
function handleUpdate(row?: WmRtVendorLine) {
  reset()
  const _id = row?.lineId || ids.value[0]
  getWmRtVendorLine(_id).then(r => { form.value = r.data; open.value = true; title.value = '修改采购退货单行表' })
}
function submitForm() {
...
function handleExport() { proxy.download('/mes/wm/rt_vendor_line/export', { ...queryParams.value }, `rtvendorline_${Date.now()}.xlsx`) }
