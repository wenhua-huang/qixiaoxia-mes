<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5"><el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['mes:wm:product_sales_detail:add']">新增</el-button></el-col>
      <el-col :span="1.5"><el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['mes:wm:product_sales_detail:edit']">修改</el-button></el-col>
      <el-col :span="1.5"><el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['mes:wm:product_sales_detail:remove']">删除</el-button></el-col>
      <el-col :span="1.5"><el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['mes:wm:product_sales_detail:export']">导出</el-button></el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList" />
    </el-row>

    <el-table v-loading="loading" :data="productsalesdetailList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="160">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['mes:wm:product_sales_detail:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['mes:wm:product_sales_detail:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script setup lang="ts" name="WmProductSalesDetail">
import { ref, reactive, toRefs, getCurrentInstance } from 'vue'
import type { WmProductSalesDetailQueryParams, WmProductSalesDetail } from '@/types/api/mes/wm/product_sales_detail'
import { listWmProductSalesDetail, getWmProductSalesDetail, delWmProductSalesDetail, addWmProductSalesDetail, updateWmProductSalesDetail } from '@/api/mes/wm/product_sales_detail'

const { proxy } = getCurrentInstance() as any
const dicts = proxy.useDict('sys_yes_no')

const productsalesdetailList = ref<WmProductSalesDetail[]>([])
...
const data = reactive({
  form: {} as WmProductSalesDetail,
  queryParams: { pageNum: 1, pageSize: 10 } as WmProductSalesDetailQueryParams,
  rules: {}
})
...
function getList() {
  loading.value = true
  listWmProductSalesDetail(queryParams.value).then(r => {
    productsalesdetailList.value = r.rows
    total.value = r.total
    loading.value = false
  })
}
...
function reset() { form.value = {} as WmProductSalesDetail; proxy.resetForm('formRef') }
...
function handleAdd() { reset(); open.value = true; title.value = '新增销售出库单明细表' }
function handleUpdate(row?: WmProductSalesDetail) {
  reset()
  const _id = row?.detailId || ids.value[0]
  getWmProductSalesDetail(_id).then(r => { form.value = r.data; open.value = true; title.value = '修改销售出库单明细表' })
}
function submitForm() {
  proxy.$refs['formRef'].validate((v: boolean) => {
    if (v) {
      const fn = form.value.detailId ? updateWmProductSalesDetail : addWmProductSalesDetail
      fn(form.value).then(() => { proxy.$modal.msgSuccess('操作成功'); open.value = false; getList() })
    }
  })
}
function handleDelete(row?: WmProductSalesDetail) {
  const _ids = row?.detailId ? [row.detailId] : ids.value
  proxy.$modal.confirm('是否确认删除？').then(() => delWmProductSalesDetail(_ids)).then(() => { getList(); proxy.$modal.msgSuccess('删除成功') })
}
function handleExport() { proxy.download('/mes/wm/product_sales_detail/export', { ...queryParams.value }, `productsalesdetail_${Date.now()}.xlsx`) }
