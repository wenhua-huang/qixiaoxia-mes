...
import { getSalesList, getSalesDetailByCode } from '@/api/mes/wm/sales'
...
function resolveCode(code) {
  ...
  getSalesDetailByCode(code.trim()).then(res => {
    uni.hideLoading()
    const d = res.data
    if (!d) { uni.showToast({ title: '出库单不存在', icon: 'none' }); return }
    proxy.$tab.navigateTo('/pages/mes/wm/sales/scan-out?salesId=' + d.salesId)
