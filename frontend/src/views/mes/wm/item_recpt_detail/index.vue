<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5"><el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['mes:wm:item_recpt_detail:add']">新增</el-button></el-col>
      <el-col :span="1.5"><el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['mes:wm:item_recpt_detail:edit']">修改</el-button></el-col>
      <el-col :span="1.5"><el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['mes:wm:item_recpt_detail:remove']">删除</el-button></el-col>
      <el-col :span="1.5"><el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['mes:wm:item_recpt_detail:export']">导出</el-button></el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList" />
    </el-row>

    <el-table v-loading="loading" :data="itemrecptdetailList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="160">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['mes:wm:item_recpt_detail:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['mes:wm:item_recpt_detail:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="600px" append-to-body>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="入库数量" prop="quantity2">
...
import type { WmItemRecptDetailQueryParams, WmItemRecptDetail } from '@/types/api/mes/wm/item_recpt_detail'
import { listWmItemRecptDetail, getWmItemRecptDetail, delWmItemRecptDetail, addWmItemRecptDetail, updateWmItemRecptDetail } from '@/api/mes/wm/item_recpt_detail'

const { proxy } = getCurrentInstance() as any
const dicts = proxy.useDict('sys_yes_no')

const itemrecptdetailList = ref<WmItemRecptDetail[]>([])
...
const data = reactive({
  form: {} as WmItemRecptDetail,
  queryParams: { pageNum: 1, pageSize: 10 } as WmItemRecptDetailQueryParams,
  rules: {}
})
...
function getList() {
  loading.value = true
  listWmItemRecptDetail(queryParams.value).then(r => {
    itemrecptdetailList.value = r.rows
    total.value = r.total
    loading.value = false
  })
}
...
function reset() { form.value = {} as WmItemRecptDetail; proxy.resetForm('formRef') }
...
function handleAdd() { reset(); open.value = true; title.value = '新增物料入库单明细表' }
function handleUpdate(row?: WmItemRecptDetail) {
  reset()
  const _id = row?.detailId || ids.value[0]
  getWmItemRecptDetail(_id).then(r => { form.value = r.data; open.value = true; title.value = '修改物料入库单明细表' })
}
function submitForm() {
  proxy.$refs['formRef'].validate((v: boolean) => {
    if (v) {
      const fn = form.value.detailId ? updateWmItemRecptDetail : addWmItemRecptDetail
      fn(form.value).then(() => { proxy.$modal.msgSuccess('操作成功'); open.value = false; getList() })
    }
  })
}
function handleDelete(row?: WmItemRecptDetail) {
  const _ids = row?.detailId ? [row.detailId] : ids.value
  proxy.$modal.confirm('是否确认删除？').then(() => delWmItemRecptDetail(_ids)).then(() => { getList(); proxy.$modal.msgSuccess('删除成功') })
}
function handleExport() { proxy.download('/mes/wm/item_recpt_detail/export', { ...queryParams.value }, `itemrecptdetail_${Date.now()}.xlsx`) }
