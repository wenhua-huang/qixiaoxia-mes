  ...
  <el-drawer v-model="visible" :title="drawerTitle" size="72%" :close-on-click-modal="false">
    <template #header>
      <div>
        ...
        <dict-tag v-if="form.rqcType" :options="mes_qc_rqc_type" :value="form.rqcType" style="margin-left: 6px" />
      </div>
    </template>

    <!-- 基本信息（只读区，头字段由来源退料单生成；退料原因/责任归属由检验员录入） -->
    <el-descriptions :column="3" border size="small" class="mb8">
      ...
      <el-descriptions-item label="批次号">{{ form.batchCode || '—' }}</el-descriptions-item>
      ...
      <el-descriptions-item label="责任归属">
        <el-select v-if="!readonly" v-model="form.responsibility" placeholder="请选择" clearable size="small" style="width: 130px">
          <el-option v-for="d in mes_qc_responsibility" :key="d.value" :label="d.label" :value="d.value" />
        </el-select>
        <el-tag v-else-if="form.responsibility" :type="respTag[form.responsibility] || 'info'" size="small">{{ respLabel(form.responsibility) }}</el-tag>
        <span v-else>—</span>
      </el-descriptions-item>
    </el-descriptions>
    ...
    <el-descriptions :column="1" border size="small" class="mb8">
      <el-descriptions-item label="退料原因">
        <el-input v-if="!readonly" v-model="form.returnReason" type="textarea" :rows="2" maxlength="500" show-word-limit placeholder="请描述退料原因" />
        <span v-else>{{ form.returnReason || '—' }}</span>
      </el-descriptions-item>
    </el-descriptions>
    ...
    <template #footer>
      ...
      <el-button v-if="!readonly && canJudge" type="warning" @click="judgeVisible = true" v-hasPermi="['mes:qc:rqc:judge']">执行判定</el-button>
      <el-button @click="visible = false">关 闭</el-button>
    </template>

    <QcDefectDialog v-model:visible="defectVisible" :defects="defectRecords" qc-type="RQC" :readonly="readonly" :lines="lines" />
...
<script setup lang="ts" name="RqcEditDrawer">
import { ref, computed } from 'vue'
import { getCurrentInstance } from 'vue'
import type { QcRqc, QcOrderLine, QcDefectRecord } from '@/api/mes/qc/rqc'
import { getRqc, updateRqc, judgeRqc } from '@/api/mes/qc/rqc'
...
const { mes_qc_status, mes_qc_check_result, mes_qc_defect_level, mes_qc_rqc_type, mes_qc_responsibility } = useDict(
  'mes_qc_status', 'mes_qc_check_result', 'mes_qc_defect_level', 'mes_qc_rqc_type', 'mes_qc_responsibility'
)
...
const drawerTitle = computed(() => `${readonly.value ? '检验单详情' : '录入检验'} - ${form.value.rqcCode || ''}`)
...
function open(rqcId: number, ro: boolean) {
  readonly.value = ro
  visible.value = true
  load(rqcId)
}

function load(rqcId: number) {
  getRqc(rqcId).then((response: any) => {
...
function save() {
  ...
  saving.value = true
  updateRqc({ ...form.value, quantityCheck: quantityCheck.value, lines: lines.value, defectRecords: defectRecords.value }).then(() => {
    proxy.$modal.msgSuccess('保存成功')
    emit('success')
    load(form.value.rqcId!)
...
function onJudge(payload: { quantityCheck: number; concessionReason?: string }) {
  ...
  updateRqc({ ...form.value, quantityCheck: payload.quantityCheck, lines: lines.value, defectRecords: defectRecords.value })
    .then(() => judgeRqc(form.value.rqcId!, payload.concessionReason))
    .then(() => getRqc(form.value.rqcId!))
    .then((r: any) => {
      form.value = r.data || {}
