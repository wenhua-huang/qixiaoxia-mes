<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5"><el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['mes:wm:misc_issue_line:add']">新增</el-button></el-col>
      <el-col :span="1.5"><el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['mes:wm:misc_issue_line:edit']">修改</el-button></el-col>
      <el-col :span="1.5"><el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['mes:wm:misc_issue_line:remove']">删除</el-button></el-col>
      <el-col :span="1.5"><el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['mes:wm:misc_issue_line:export']">导出</el-button></el-col>
    ...
    <el-table v-loading="loading" :data="miscissuelineList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="160">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['mes:wm:misc_issue_line:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['mes:wm:misc_issue_line:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="600px" append-to-body>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="杂项出库单ID" prop="issueId">
          <el-input v-model="form.issueId" placeholder="请输入" clearable />
        </el-form-item>
...
<script setup lang="ts" name="WmMiscIssueLine">
import { ref, reactive, toRefs, getCurrentInstance } from 'vue'
import type { WmMiscIssueLineQueryParams, WmMiscIssueLine } from '@/types/api/mes/wm/misc_issue_line'
import { listWmMiscIssueLine, getWmMiscIssueLine, delWmMiscIssueLine, addWmMiscIssueLine, updateWmMiscIssueLine } from '@/api/mes/wm/misc_issue_line'

const { proxy } = getCurrentInstance() as any
const dicts = proxy.useDict('sys_yes_no')

const miscissuelineList = ref<WmMiscIssueLine[]>([])
...
const data = reactive({
  form: {} as WmMiscIssueLine,
  queryParams: { pageNum: 1, pageSize: 10 } as WmMiscIssueLineQueryParams,
  rules: {}
})
...
function getList() {
  loading.value = true
  listWmMiscIssueLine(queryParams.value).then(r => {
    miscissuelineList.value = r.rows
    total.value = r.total
    loading.value = false
  })
}
...
function reset() { form.value = {} as WmMiscIssueLine; proxy.resetForm('formRef') }
...
function handleAdd() { reset(); open.value = true; title.value = '新增杂项出库单行表' }
function handleUpdate(row?: WmMiscIssueLine) {
  reset()
  const _id = row?.lineId || ids.value[0]
  getWmMiscIssueLine(_id).then(r => { form.value = r.data; open.value = true; title.value = '修改杂项出库单行表' })
}
function submitForm() {
  proxy.$refs['formRef'].validate((v: boolean) => {
    if (v) {
      const fn = form.value.lineId ? updateWmMiscIssueLine : addWmMiscIssueLine
      fn(form.value).then(() => { proxy.$modal.msgSuccess('操作成功'); open.value = false; getList() })
    }
  })
}
function handleDelete(row?: WmMiscIssueLine) {
  const _ids = row?.lineId ? [row.lineId] : ids.value
  proxy.$modal.confirm('是否确认删除？').then(() => delWmMiscIssueLine(_ids)).then(() => { getList(); proxy.$modal.msgSuccess('删除成功') })
}
function handleExport() { proxy.download('/mes/wm/misc_issue_line/export', { ...queryParams.value }, `miscissueline_${Date.now()}.xlsx`) }
