---
description: 启动前后端本地开发环境 — 检查 Docker 依赖 → 构建后端 JAR → 启动后端 :8081 → 启动前端 :5173 → 验证全链路
---
...
```bash
cd /Users/huangwenhua/company/self/qixiaoxia-mes/backend
...
```bash
cd /Users/huangwenhua/company/self/qixiaoxia-mes/frontend
...
# 前端：仅重启 Vite
lsof -ti :5173 | xargs kill 2>/dev/null
cd /Users/huangwenhua/company/self/qixiaoxia-mes/frontend && npm run dev &
```
