// Calendar utility — 农历/节假日辅助
// solar2lunar 使用简化的公历→农历转换
...
function leapYear(y: number) { return (y % 4 === 0 && y % 100 !== 0) || y % 400 === 0 }
...
export function solar2lunar(y: number, m: number, d: number) {
  ...
  for (i = 1; i < 13 && offset > 0; i++) {
    if (isLeap > 0 && i === isLeap + 1 && !isLeapMonth) {
      --i
      isLeapMonth = true
      temp = leapDays(year)
    } else {
    ...
    if (isLeapMonth && i === isLeap + 1) isLeapMonth = false
    offset -= temp
  }

  if (offset === 0 && isLeap === i && !isLeapMonth) {
    isLeapMonth = true
  } else if (offset < 0) {
    offset += temp; i--
  }
  ...
  return {
    ...
    lDay: day,
    isLeap: isLeapMonth,
    IMonthCn: (isLeapMonth ? '闰' : '') + lunarMonthNames[month],
    IDayCn: lunarDayNames[day],
    festival: '',
    lunarFestival: '',
    Term: '',
  }
...
export default {
  ...
  getDays(date: Date) {
    ...
    for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
      days.push({ theDay: new Date(d).toISOString().split('T')[0], shiftType: '', teamShifts: [] })
    }
    return days
  },
  solar2lunar,
