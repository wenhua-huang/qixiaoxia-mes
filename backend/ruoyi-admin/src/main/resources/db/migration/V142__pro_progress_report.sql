-- V142: 工单进度跟踪与统计 —— 班组快照/索引/菜单/配置
--
-- 注：ALTER/CREATE INDEX 用 PROCEDURE 包装实现幂等（V142 曾在本地因 collation 失败，
--     部分列已添加，重跑需跳过）。sys_user 是 utf8mb4_general_ci，与业务表
--     utf8mb4_unicode_ci 在 user_name 上比较需显式 COLLATE，否则 1267。
...
CALL add_col_if_missing('qxx_pro_feedback', 'user_id',
    'ALTER TABLE qxx_pro_feedback ADD COLUMN user_id BIGINT NULL COMMENT ''报工人用户ID(关联sys_user)'' AFTER user_name');
CALL add_col_if_missing('qxx_pro_feedback', 'team_id',
    'ALTER TABLE qxx_pro_feedback ADD COLUMN team_id BIGINT NULL COMMENT ''班组ID(快照)'' AFTER user_id');
CALL add_col_if_missing('qxx_pro_feedback', 'team_code',
    'ALTER TABLE qxx_pro_feedback ADD COLUMN team_code VARCHAR(64) NULL COMMENT ''班组编码(快照)'' AFTER team_id');
CALL add_col_if_missing('qxx_pro_feedback', 'team_name',
    'ALTER TABLE qxx_pro_feedback ADD COLUMN team_name VARCHAR(128) NULL COMMENT ''班组名称(快照)'' AFTER team_code');

-- 2. 上下工：班组快照（已有 user_id）
CALL add_col_if_missing('qxx_pro_workrecord', 'team_id',
    'ALTER TABLE qxx_pro_workrecord ADD COLUMN team_id BIGINT NULL COMMENT ''班组ID(快照)'' AFTER task_code');
CALL add_col_if_missing('qxx_pro_workrecord', 'team_code',
    'ALTER TABLE qxx_pro_workrecord ADD COLUMN team_code VARCHAR(64) NULL COMMENT ''班组编码(快照)'' AFTER team_id');
CALL add_col_if_missing('qxx_pro_workrecord', 'team_name',
    'ALTER TABLE qxx_pro_workrecord ADD COLUMN team_name VARCHAR(128) NULL COMMENT ''班组名称(快照)'' AFTER team_code');

DROP PROCEDURE IF EXISTS add_col_if_missing;

-- 3. 历史回填：feedback.user_id 按 user_name 解析（显式 COLLATE 对齐排序规则）
UPDATE qxx_pro_feedback f
  JOIN sys_user u ON u.user_name COLLATE utf8mb4_unicode_ci = f.user_name AND u.del_flag = '0'
...
-- 6. 索引
CALL add_index_if_missing('qxx_pro_card_process', 'idx_card_process_task_io',
    'CREATE INDEX idx_card_process_task_io ON qxx_pro_card_process(task_id, input_time, output_time)');
CALL add_index_if_missing('qxx_pro_card_process', 'idx_card_process_card_seq',
    'CREATE INDEX idx_card_process_card_seq ON qxx_pro_card_process(card_id, seq_num)');
CALL add_index_if_missing('qxx_pro_feedback', 'idx_feedback_time_team',
    'CREATE INDEX idx_feedback_time_team ON qxx_pro_feedback(feedback_time, team_id)');
CALL add_index_if_missing('qxx_pro_workrecord', 'idx_workrecord_clock_team',
    'CREATE INDEX idx_workrecord_clock_team ON qxx_pro_workrecord(clock_in_time, team_id)');
CALL add_index_if_missing('qxx_pro_task', 'idx_task_end_status',
...
-- 9. 授权 admin 角色（sys_role_menu 主键含 factory_id，Flyway 裸 JDBC 必须显式写）
INSERT IGNORE INTO sys_role_menu (role_id, menu_id, factory_id)
SELECT 1, menu_id, 0 FROM sys_menu WHERE menu_id = 2320;
